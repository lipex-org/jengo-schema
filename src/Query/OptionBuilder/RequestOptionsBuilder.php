...
final class RequestOptionsBuilder
{
    ...
    public static function build(?QueryOptions $options = null): QueryOptions
    {
        ...
        if ($allowAll || in_array('where', $allowed, true)) {
            $orWhere       = (bool) $request->getGet('orWhere');
        ...
        // 4. Search
        $search = $options?->search ?? new SearchOptions();
        if ($allowAll || in_array('search', $allowed, true)) {
            $searchValue = $request->getGet('search');
            if ($searchValue !== null && $searchValue !== '') {
                $search = new SearchOptions(value: $searchValue);
            }
        }
    ...
    private function parseWhere(array $wheres, bool $isAndWhere = true): array
    {
        ...
        foreach ($wheres as $key => $value) {
            if (! in_array($key, $this->reservedQueryWords, true)) {
    ...
    private function unwrapReservedWord(string $value): ?string
    {
    ...
    private function parseSelectValue(string $value, string $key, bool $isAndWhere = true): array
    {
        // before callbacks
        ...
        if (! in_array($value, $this->reservedQueryValues, true)) {
        }
